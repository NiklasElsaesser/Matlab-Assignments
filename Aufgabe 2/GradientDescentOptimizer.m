...
%             if ~exist('obj.learningRate','var')
%                 error('Error obj.learningRate');
%             end
%             if ~exist('obj.maxIterations','var')
%                 error('Error obj.maxIterations');
%             end
        end
        ...
        function h = runTraining(obj, linearRegressionModel)
            ...
            for i = 1:maxIters
                theta = [theta(1) - alpha * (1 / m * sum(X * theta - y)); theta(2) - alpha * (1 / m * sum((X * theta - y) .* X(:, 2)))];
